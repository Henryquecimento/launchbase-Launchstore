...
CREATE TRIGGER trgger_set_timestamp
...
--1. create a column in products as deleted_at
ALTER TABLE products ADD COLUMN "deleted_at" timestamp;

--2.crate a RULE that it'll run everytime we request the DELETE
...
--3.Create a VIEW where we'll take just the active data(non deleted data)
CREATE VIEW products_without_deleted AS
SELECT * FROM products WHERE deleted_at IS NULL;

--4.RENAME THE VIEW AND THE PRODUCT TABLE
